...
use crate::primes::{compute_with_memo, PrimeType};
use eframe::egui;
use std::thread;
...
fn run_once(kind: PrimeType, start: u64, end: u64) -> (String, f32) {
    ...
    spawn_worker(kind, start, end, stop, out.clone(), prog.clone(), egui_ctx);

    // wait (poll) until progress == 1.0 or text reports stop
    while *prog.lock().unwrap() < 1.0 && !out.lock().unwrap().contains("stopped") {
...
fn empty_range_test() {
    ...
    assert_eq!(
        progress, 1.0,
        "Progress should complete even for empty range"
    );
}
...
fn single_element_range_test() {
    ...
    assert!(
        txt.contains("11"),
        "Single element range should find prime 11"
    );
    assert_eq!(
        progress, 1.0,
        "Progress should complete for single element range"
    );
}
...
fn boundary_values_test() {
    ...
    assert!(
        txt.contains("3"),
        "3 should be identified as a Mersenne prime"
    );
}
...
fn adjacent_ranges_test() {
    ...
    assert_eq!(
        combined_range, expected_combined,
...
fn very_large_start_value_test() {
    ...
    assert_eq!(
        progress, 1.0,
        "Progress should complete for large start values"
    );
}
...
fn different_families_same_range() {
    ...
    assert!(
        !cousin.is_empty(),
        "Cousin primes should exist in range 20-40"
    );
