...
use gui_primes::primes::{compute_with_memo, PrimeType};
...
fn test_range_swapping() {
    ...
    assert_eq!(
        ...
        "Swapped ranges should produce identical results"
    );
}
...
fn test_memoization() {
    ...
    assert_eq!(
        ...
        "Memoized results should be identical"
    );
...
fn test_edge_cases() {
    ...
    assert!(
        ...
        "Empty range should produce no results"
    );
    ...
    assert!(
        ...
        "Range with only composite numbers should return empty result"
    );
}
